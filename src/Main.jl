...
function (@main)(args)
    ...
    if parsed_args["%COMMAND%"] == "diagonalize"
        ...
        if cluster_idx < 1 || cluster_idx > length(keys(cluster_data))
            error(
                "Cluster index $cluster_idx out of bounds! File contains $(length(cluster_data["clusters"])) clusters.",
            )
        end
    ...
    elseif parsed_args["%COMMAND%"] == "merge"
        ...
        error(
            "Invalid State! 'graph' is not defined and command is not 'merge'. This should never happen!",
        )
    end
...
function ArgParse.parse_item(::Type{Logging.LogLevel}, s::AbstractString)
    ...
    elseif level == "min" || level == "all" || level == "belowminlevel"
        return Logging.BelowMinLevel
    elseif level == "max" || level == "none" || level == "abovemaxlevel"
